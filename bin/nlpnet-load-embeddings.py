...
def clusterize_words(model, _filter=None):
    '''
    ...
    for word, vector in model.items():
        if (_filter and _filter(word)) or word.strip() == '':
            continue
...
def read_skipdep_embeddings(filename):
    ...
    clusters = clusterize_words(model, _filter=dep_filter)
...
def read_gensim_embeddings(filename):
    ...
    extra_vectors = nlpnet.utils.generate_feature_vectors(
        num_extra_vectors, num_features
    )
...
This script can deal with the following formats:
    ...
    parser.add_argument(
        'type', help='Format of the embeddings. See the description below.',
        choices=[
            'plain', 'senna', 'gensim', 'word2embeddings', 'single', 'polyglot'
        ]
    )
    parser.add_argument(
        'embeddings', help='File containing the actual embeddings'
    )
