'''
This script will run a POS or SRL tagger
on the input data and print the results to stdout.
'''
...
def interactive_running(args):
    ...
    It receives text from the standard input, tokenizes it,
    and calls the function given as a parameter to produce an answer.
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'task', help='Task for which the network should be used.',
        type=str, choices=['srl', 'pos', 'dependency'])
    parser.add_argument(
        '--data',
        help='Directory containing trained models (default: current)',
        type=str, default='.')
    parser.add_argument(
        '-v', help='Verbose mode', action='store_true', dest='verbose')
    parser.add_argument(
        '-t', action='store_true', dest='disable_tokenizer',
        help='Disable built-in tokenizer. Tokens are assumed'
             ' to be separated by whitespace.')
    parser.add_argument(
        '--lang', dest='lang', default='en',
        help='Language (used to determine which tokenizer to run.'
             ' Ignored if -t is provided)', choices=['en', 'pt'])
    parser.add_argument(
        '--no-repeat', dest='no_repeat', action='store_true',
        help='Forces the classification step to avoid '
             'repeated argument labels (SRL only)')
