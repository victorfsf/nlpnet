...
from nlpnet.network import ConvolutionalDependencyNetwork
from nlpnet.network import ConvolutionalNetwork
...
def create_reader(args, md, validation=False):
    ...
    if args.task == 'pos':
        ...
        text_reader = srl.SRLReader(
            md, filename=filename, only_boundaries=args.identify,
...
def create_network(args, text_reader, feature_tables, md):
    ...
    is_convolution_srl = args.task.startswith('srl') and \
    ...
    if is_convolution:
        ...
        distance_tables = utils.set_distance_features(
            args.max_dist, args.target_features, args.pred_features)
        ...
        if is_dependency:
            ...
            if args.identify:
                logger.info(
                    'Loading initial transition scores '
                    'table for argument identification')
        ...
        # not convolution
        num_tags = len(text_reader.tag_dict)
        nn = Network.create_new(
            feature_tables, args.window, args.hidden, num_tags)
...
def create_metadata(args):
    ...
    # using getattr because the SRL args object
    # doesn't have a 'suffix' attribute
...
def train(nn, reader, args):
    ...
    logger.debug(
        'Maximum weight norm set to %.2f (0 means disabled)' % nn.max_norm)
    logger.debug('----------------------------------------------------\n')
    ...
    if args.task.startswith('srl') and args.task != 'srl_predicates':
        arg_limits = (
            None if args.task != 'srl_classify' else text_reader.arg_limits
        )
