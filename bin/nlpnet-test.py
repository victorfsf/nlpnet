...
def evaluate_pos(gold_file=None, oov=None):
    ...
    :param oov: either None or a list of tokens,
                that should contain the oov words.
    '''
...
def evaluate_unlabeled_dependency(gold_file, punctuation):
    ...
    print('%d sentences completely correct (%f%%)' % (
        sentence_hits, sent_accuracy))
    print('Accuracy: %f%%' % (100 * accuracy))
...
def evaluate_labeled_dependency(gold_file, punctuation):
    ...
    for sent, heads, labels in zip(
            reader.sentences, reader.heads, reader.labels):
...
def sentence_precision(
        network_tags, gold_tags, gold_tag_dict, network_tag_dict):
    '''
...
def evaluate_srl_classify(no_repeat=False, gold_file=None):
    ...
    for sentence, tags, predicates, args in zip(
            r.sentences, r.tags, r.predicates, r.arg_limits):

        # the answer includes all predicates
        answer = nn.tag_sentence(
            sentence, predicates, args, allow_repeats=not no_repeat)
...
def evaluate_srl_2_steps(
        no_repeat=False, find_preds_automatically=False, gold_file=None):
    '''
    ...
    for sent in actual_sentences:
        ...
        boundaries = [
            [itd_boundary[x] for x in pred_answer] for pred_answer in answers
        ]
        ...
        arguments = [
            [itd_classify[x] for x in pred_answer] for pred_answer in answers
        ]
...
def evaluate_srl_1step(find_preds_automatically=False, gold_file=None):
    ...
    for sent in iter(r.sentences):
        ...
        verbs = [
            (position, actual_sent[position].word)
            for position in pred_positions
        ]
        sent_codified = np.array([
            r.converter.convert(token) for token in actual_sent
        ])
...
def evaluate_srl_identify(gold_file):
    ...
    for sent, preds, sent_tags in zip(
            srl_reader.sentences, srl_reader.predicates, srl_reader.tags):

        # one answer for each predicate
        answers = nn.tag_sentence(sent, preds)

        for answer, tags in zip(answers, sent_tags):
            correct_args, existing_args = sentence_recall(
                answer, tags, gold_itd, net_itd)
            counter_correct_args.update(correct_args)
            counter_existing_args.update(existing_args)

            _, predicted_args = sentence_precision(
                answer, tags, gold_itd, net_itd)
    ...
    for arg in counter_existing_args:
        ...
        # - if the network never tagged a given argument,
        # its precision is 100% (it never made a mistake)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Test nlpnet model perfomance')
    ...
    base_parser.add_argument(
        '-v', help='Verbose mode', action='store_true', dest='verbose')
    base_parser.add_argument(
        '--gold', help='File with gold standard data', type=str, required=True)
    base_parser.add_argument(
        '--data',
        help='Directory with trained models (default: current directory)',
        type=str, default='.')

    parser_pos = subparsers.add_parser(
        'pos', help='POS tagging', parents=[base_parser])
    parser_srl = subparsers.add_parser(
        'srl', help='Semantic Role Labeling', parents=[base_parser])
    parser_dep = subparsers.add_parser(
        'dependency', help='Dependency parsing', parents=[base_parser])

    parser_srl.add_argument(
        '--id', help='Evaluate only argument identification',
        action='store_true', dest='identify')
    parser_srl.add_argument(
        '--class', help='Evaluate only argument classification',
        action='store_true', dest='classify')
    parser_srl.add_argument(
        '--preds', help='Evaluate only predicate identification',
        action='store_true', dest='predicates')
    parser_srl.add_argument(
        '--2steps', help='Execute SRL with two separate steps',
        action='store_true', dest='two_steps')
    parser_srl.add_argument(
        '--no-repeat', dest='no_repeat', action='store_true',
        help='Forces the classification step to avoid repeated'
             ' argument labels (2 step SRL only).')
    parser_srl.add_argument(
        '--auto-pred', dest='auto_pred', action='store_true',
        help='Determines SRL predicates automatically '
             '(instead of gold annotation)')

    parser_dep.add_argument(
        'type', help='Whether to test labeled or unlabeled performance',
        choices=['labeled', 'unlabeled'])
    parser_dep.add_argument(
        '-p', help='Score on punctuation (ignored by default)',
        action='store_true', dest='punctuation')

    parser_pos.add_argument(
        '--oov', help='Analyze performance on OOV data. '
                      'Not fully functional with numbers.', type=str)
    ...
    logger.info(
        'Total time (including reading data): {:.2f}s'.format(time_diff)
    )
