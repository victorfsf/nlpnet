...
def get_args():
    parser = argparse.ArgumentParser(
        description='Train nlpnet for a given NLP task.')
    subparsers = parser.add_subparsers(
        title='Tasks', dest='task',
    ...
    base_parser.add_argument(
        '--gold', help='File with annotated data for training.',
        type=str, required=True)
    base_parser.add_argument(
        '--data', help='Directory to save new models and load '
        'partially trained ones (default: current directory)', type=str,
        default='.')
    base_parser.add_argument(
        '--dev', help='Development (validation) data. If not given, '
        'training data will be used to evaluate performance.', default=None)
    ...
    network_parser.add_argument(
        '--max-norm', type=float, default=0, dest='max_norm',
        help='Maximum weight norm; limits '
             'the norm of weight vectors (default 0 ie disabled)')
    network_parser.add_argument(
    ...
    network_parser.add_argument('-n', '--hidden', type=int,
                                ...
                                dest='hidden')
    network_parser.add_argument(
        '--caps', const=5, nargs='?', type=int, default=None,
    ...
    conv_parser.add_argument(
        '--pos', const=5, nargs='?', type=int, default=None,
        help='Include part-of-speech features. '
             'Optionally, supply the number of features (default 5)')
    conv_parser.add_argument(
        '--max-dist', type=int, default=10, dest='max_dist',
        help='Maximum distance to have its own feature vector (default 10)')
    conv_parser.add_argument(
        '--target-features', type=int, default=5, dest='target_features',
        help='Number of features for distance to target word (default 5)')
    conv_parser.add_argument(
        '--pred-features', type=int, default=5, dest='pred_features',
    ...
    dep_subparsers = parser_dep.add_subparsers(
        title='Dependency parsing training steps',
        dest='subtask',
    ...
    parser_srl = subparsers.add_parser(
        'srl', help='Semantic Role Labeling',
        formatter_class=argparse.RawDescriptionHelpFormatter)
    parser_srl.set_defaults(identify=False, predicates=False, classify=False)

    desc = '''SRL has 3 steps: predicate  detection, argument
identification and argument classification. Each one depends on the one before.
...
Type %(prog)s [SUBTASK] -h to get subtask-specific help.'''
    ...
    defaults['srl'] = dict(window=3, hidden=150, convolution=200,
                           iterations=15, learning_rate=0.001,
                           learning_rate_features=0.001,
                           learning_rate_transitions=0.001)
