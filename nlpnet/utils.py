...
_tokenizer_regexp = r'''(?ux)
    ...
    (?:[DSds][Rr][Aa]?)\.|      # common abbreviations such as dr., sr., sra., dra.  # noqa
    (?:[^\W\d_]){1,2}\$|        # currency
    (?:[\#@]\w+])|              # Hashtags and twitter user names
    -(?:[^\W\d_])+|             # clitic pronouns with leading hyphen
    \w+(?:[-']\w+)*|            # words with hyphens or apostrophes, e.g. não-verbal, McDonald's  # noqa
...
def tokenize_pt(text, clean=True):
    ...
    :param clean: If True, performs some cleaning action on the text,
                  such as replacing all digits for 9
                  (by calling :func:`clean_text`)
    '''
    if clean:
    ...
    # loads trained model for tokenizing PT sentences (provided by NLTK)
    sent_tokenizer = nltk.data.load('tokenizers/punkt/portuguese.pickle')
    sentences = [
        _tokenizer.tokenize(sent)
...
def clean_text(text, correct=True):
    ...
    if correct:
        ...
        # inserts space after leading hyphen.
        # It happens sometimes in cases like:
...
def contract(w1, w2):
    ...
    If a contraction between the given words doesn't exist in Portuguese,
    ...
    if contraction is None:
        raise ValueError(
            'Unexpected contraction: \'%s\' and \'%s\'' % (w1, w2))

    return attributes.capitalize(contraction, cap)


def generate_feature_vectors(
        num_vectors, num_features, min_value=-0.1, max_value=0.1):
    '''
    ...
    table = (max_value * 2) * \
        np.random.random((num_vectors, num_features)) + min_value
    logger.debug('Generated %d feature vectors with %d features each.' % (
...
def _create_affix_tables(affix, table_list, num_features):
    ...
    num_affixes_per_size = getattr(
        attributes.Affix, 'num_%ses_per_size' % affix)
    for size in codes:
...
def create_feature_tables(args, md, text_reader):
    ...
    if not args.load_types:
        ...
        if len(types_table) < len(text_reader.word_dict):
            ...
            logger.warning(
                'Number of features provided is greater than the'
                ' number of tokens in the dictionary.'
                ' The extra features will be ignored.'
            )
    ...
    if md.use_caps:
        ...
        caps_table = generate_feature_vectors(
            attributes.Caps.num_values, args.caps)
...
def make_contractions_srl(sentences, predicates):
    '''
    Makes preposition contractions in the
    input data for SRL with Portuguese text.
    ...
    :returns: a tuple (sentences, predicates)
              after contractions have been made.
    '''
    ...
    for (sent, props), preds in zip(sentences, predicates):
        for i, token in enumerate(sent):
            ...
            # look at the arg types for this and
            # the next token in all propostions
            ...
            # check if the tags for this token
            # and the next are the same in all propositions
            # if the first is O, however, we will merge them anyway.
            if all(a1 == a2 or a1 == 'O'
                    for a1, a2 in zip(arg_types, next_arg_types)):

                if word == 'de' and next_word in (
                        def_articles + pronouns + pronouns_a + adverbs):
                    contract('d' + next_word, 'd' + next_token.lemma)

                elif word == 'em' and next_word in (
                        def_articles + pronouns + pronouns_a):
                    ...
                    if next_word in pronouns_a:
                        contract(
                            u'à' + next_word[1:], u'à' + next_token.lemma[1:])
