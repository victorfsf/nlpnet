...
class SRLReader(reader.TaggerReader):
    ...
    def _read_conll(self, filename):
        ...
        for line in lines:
            ...
            # find out how many predicates are there.
            # initialize a list for each of them.
            if sent_tags == []:
                expected_roles = []
                for tag in tags:
    ...
    def load_tag_dict(self, filename=None, iob=False):
        ...
        The dictionary file should have one tag per line.
    ...
    def _generate_iobes_dictionary(self):
        '''
        Generate the reader's tag dictionary
        mapping the IOBES tags to numeric codes.
        '''
    ...
    def generate_dictionary(self, dict_size=None, minimum_occurrences=2):
        ...
        :param dict_size: Max number of tokens to be
    ...
    def create_converter(self):
        '''
        This function overrides the TextReader's
        one in order to deal with Token
        objects instead of raw strings.
        '''
        self.converter = attributes.TokenConverter()
        self.converter.add_extractor(
            lambda t: self.word_dict.get(
                t.lemma if self.md.use_lemma else t.word
            )
        )
        if self.md.use_caps:
            self.converter.add_extractor(
        ...
        if self.md.use_chunk:
            ...
            self.converter.add_extractor(lambda t: chunk_def_dict[t.chunk])
    ...
    def _remove_tag_names(self):
        '''
        Removes the actual tag names,
        leaving only IOB or IOBES block delimiters.
        '''
    ...
    def _codify_sentences(self):
        ...
        for (sent, props), preds in zip(self.sentences, self.predicates):
            ...
            if self.task == 'srl_predicates':
                ...
                for prop in props:
                    # for classifying arguments, leave the names.
    ...
    def codify_sentences(self):
        '''
        Converts each token in each sequence into indices to their
        feature vectors in feature matrices.
        The previous sentences as text are not accessible anymore.
        Tags are also encoded.
        This function takes care of the case of classifying
    ...
    def convert_tags(self, scheme, update_tag_dict=True,
                     only_boundaries=False):
        '''
        Replaces each word label with an IOB or IOBES version,
        appending a prefix to them.
        ...
        if only_boundaries:
            ...
            actual_tagset = {
                tag for _, props in self.sentences
