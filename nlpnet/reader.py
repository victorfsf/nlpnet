...
class TaggerReader(object):
    ...
    def load_or_create_tag_dict(self):
        ...
        If the dict file is not available,
    ...
    def generate_dictionary(self, dict_size=None, minimum_occurrences=2):
        ...
        logger.info(
            'Created dictionary with %d types' % self.word_dict.num_tokens)
    ...
    def codify_sentences(self):
        '''
        Converts each token in each sequence
        into indices to their feature vectors in feature matrices.
    ...
    def get_word_counter(self):
        ...
        '''
        c = Counter(
            token.lower() for sent in self.sentences for token, _ in sent
        )
        return c
    ...
    def add_text(self, text):
        '''
        Adds more text to the reader.
        The text must be a sequence of sequences of tokens.
        '''
    ...
    def create_affix_list(self, prefix_or_suffix, max_size, min_occurrences):
        ...
        Check if there exists an affix list in the data directory.
        If there isn't,
        ...
        logger.info('Created a list of %d %ses.' % (
            len(affixes_all_lengths), affix_type
        ))
    ...
    def create_converter(self):
        '''
        Sets up the token converter, which is responsible
        for transforming tokens into their
        ...
        def add_affix_extractors(affix):
            ...
            # deal with gaps between sizes
            # (i.e., if there are sizes 2, 3, and 5)
            ...
            for size in sizes:
                ...
                # size=size because if we don't use it,
                # lambda sticks to the last value of the loop iterator size
