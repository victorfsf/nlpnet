...
from .network import ConvolutionalDependencyNetwork
...
def _group_arguments(tokens, predicate_positions, boundaries, labels):
    '''
    Groups words pertaining to each argument and
    returns a dictionary for each predicate.
    '''
    arg_structs = []

    for predicate_position, pred_boundaries, pred_labels in zip(
            predicate_positions, boundaries, labels):
        structure = {}
...
class SRLAnnotatedSentence(object):
    ...
    It stores a list with the sentence tokens, called `tokens`,
    and a list of tuples in the format `(predicate, arg_strucutres)`.
    Each `arg_structure` is a dict mapping semantic roles
    to the words that constitute it.
    This is used instead of a two-level dictionary because one sentence
    may have more than one occurrence of the same predicate.
    ...
    def __init__(self, tokens, arg_structures):
        ...
        :param arg_structures: a list of tuples in the format
            (predicate, mapping).
            Each predicate is a string and each mapping is a
            dictionary mapping role labels
            to the words that constitute it.
        '''
        self.tokens = tokens
...
class ParsedSentence(object):
    ...
    It stores a list of tokens, the dependency heads,
    dependency labels and POS tags if the parser used them.
    Dependency heads are the index of the head of each
    ...
    def to_conll(self):
        ...
        for i in range(len(self.tokens)):
            ...
            result.append(line.format(
                id=i+1, pos=pos, head=head, label=label, token=token))
...
class Tagger(object):
    ...
    def __init__(self, data_dir=None, language='en'):
        ...
        asrt_msg = 'nlpnet data directory is not set.' \
                   'If you don\'t have the trained models, ' \
                   'download them from ' \
...
class SRLTagger(Tagger):
    ...
    def tag_tokens(self, tokens, no_repeats=False):
        ...
        :returns: a list of lists (one list for each sentence).
            Sentences have tuples (all_tokens, predicate, arg_structure),
            where arg_structure is a dictionary
            mapping argument labels to the words it includes.
        '''
        if self.language == 'pt':
            tokens_obj = [
                attributes.Token(utils.clean_text(t, False)) for t in tokens
            ]
        else:
        ...
        answers = self.boundary_nn.tag_sentence(
            converted_bound, pred_positions)
        ...
        structures = _group_arguments(
            tokens, pred_positions, boundaries, arguments)
...
class DependencyParser(Tagger):
    ...
    def _load_data(self):
        ...
        md_udep = Metadata.load_from_file(
            'unlabeled_dependency', paths=self.paths)
        self.unlabeled_nn = load_network(md_udep)
        self.unlabeled_reader = create_reader(md_udep)

        md_ldep = Metadata.load_from_file(
            'labeled_dependency', paths=self.paths)
