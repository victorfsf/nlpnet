...
class DependencyReader(reader.TaggerReader):
    ...
    def __init__(self, md=None, filename=None, labeled=False):
        ...
        :param labeled: (ignored if md is supplied) whether it is intended
            ...
            True, another reader object will
            be needed for unlabeled dependency.
        '''
        if md is not None:
    ...
    def _read_conll(self, filename):
        ...
        for line in lines:
            ...
            if head == 0:
                # we represent a dependency to root
                # as an edge to the token itself
    ...
    def _create_pos_dict(self):
        '''
        Examine all POS tags in the sentences
        and create a dictionary based on them.
        '''
    ...
    def load_or_create_tag_dict(self):
        '''
        Try to load the tag dictionary from the default location.
        If the dictinaty file is not available,
        scan the available sentences and create a new one.
    ...
    def codify_sentences(self):
        ...
        self.sentences = [
            np.array([
                self.converter.convert(token) for token in sent
            ]) for sent in self.sentences
        ]

        if self.labeled:
            self.labels = [
                np.array([self.tag_dict[label] for label in sent_labels])
    ...
    def _load_or_create_pos_dict(self):
        ...
        if os.path.isfile(self.md.paths['dependency_pos_tags']):
            ...
            self.save_tag_dict(
                self.md.paths['dependency_pos_tags'], self.pos_dict)
    ...
    def create_converter(self):
        ...
        It also allows POS as an attribute.
        '''
        self.converter = attributes.TokenConverter()
        self.converter.add_extractor(lambda token: self.word_dict[token.word])
        ...
        if self.md.use_pos:
            self._load_or_create_pos_dict()
            self.converter.add_extractor(
                lambda token: self.pos_dict[token.pos])
