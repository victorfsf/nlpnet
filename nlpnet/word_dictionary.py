...
class WordDictionary(dict):
    ...
    def __init__(self, tokens, size=None,
                 minimum_occurrences=None, wordlist=None):
        """
        ...
        :param minimum_occurrences: The minimum number of occurrences a
            token must have in order to be included.
        :param wordlist: Use this list of words to build the dictionary.
            Overrides tokens if not None and ignores maximum size.
        """
        if wordlist is None:
            ...
            words = [
                word.lower().translate(WordDictionary.number_transformation)
    ...
    def init_from_wordlist(cls, wordlist):
        ...
        Every word in the list gets an entry.
        """
    ...
    def _get_frequency_count(self, token_list):
        ...
        if type(token_list[0]) == list:
            c = Counter(
                t.lower().translate(WordDictionary.number_transformation)
    ...
    def update_tokens(
            self, tokens, size=None, minimum_occurrences=1, freqs=None):
        """
        ...
        candidate_tokens = dict((token, freqs[token])
                                for token in freqs
                                if token not in self and
                                freqs[token] >= minimum_occurrences)

        # order the types from the most frequent to the least
        new_tokens = sorted(
            candidate_tokens, key=lambda x: candidate_tokens[x], reverse=True)
    ...
    def __contains__(self, key):
        ...
        """
        transformed = key.lower().translate(
            WordDictionary.number_transformation)
    ...
    def __getitem__(self, key):
        ...
        1) when given a word without an entry,
            it returns the value for the *RARE* key.
    ...
    def get(self, key):
        """
        Overrides the dictionary get method, so when given a word
        without an entry, it returns the value for the *RARE* key.
        Note that it is not possible to supply a default value as
        in the dict class.
        """
        # faster than regexp
        transformed = key.lower().translate(
    ...
    def check(self):
        """
        Checks the internal structure of the
        dictionary and makes necessary adjustments,
